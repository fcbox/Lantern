...
open class Lantern: UIViewController, UIViewControllerTransitioningDelegate, UINavigationControllerDelegate {
    ...
    /// 导航条是否隐藏，默认为true
    open var isNavigationBarHidden = true

    /// 上面的导航条和下面的toolbar是否隐藏
    open var isMenuHidden: Bool = false {
        didSet {
            self.navigationController?.setNavigationBarHidden(isMenuHidden, animated: true)
            self.navigationController?.setToolbarHidden(isMenuHidden, animated: true)
        }
    }
    ...
    /// 拖动图片，背景颜色变化
    open var maskAlphaChaged: (_ lantern: Lantern, _ alpha: CGFloat) -> Void = { _,_  in }
    ...
    /// 单击图片的效果，默认为隐藏视图
    open var tapStyle = TapStyle.dismiss

    /// 即将消失的事件
    open var willDismiss: ((_ lantern: Lantern) -> Void)?
